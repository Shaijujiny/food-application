...
from sqlalchemy import String, Integer, Float, ForeignKey, Boolean, func
...
class TblFoods(Base):
    ...
    async def get_by_category(cls, category_id: int, db: AsyncSession):
        result = await db.execute(
            select(cls).where(cls.category_id == category_id)
        )
    ...
    async def active_foods_count(cls, db: AsyncSession):
        result = await db.scalar(
            select(func.count(cls.food_id))
            .where(cls.is_available == True)
        )
        return result or 0